...
| **Coronavirus Dashboard UX** — psychological amplification through colour | Red carries a death association and amplifies affect out of proportion to the underlying risk; the fix is a neutral palette matched to actual severity. | A −25% revenue movement is serious but routine. Painting the whole workspace red trains the reader to panic at every screen and then to ignore all of them. | One restrained accent per semantic state. Red marks **the adverse direction**: a driver that pushed the KPI down, evidence that contradicts the leading hypothesis, or a hard verification violation. Green marks the favourable direction. The movement figure itself is neutral ink with a directional arrow; magnitude is carried by the number, not by alarm colour. → `ui/theme.py`, `ui/components/drivers.py` |
...
| Colour discipline (COVID dashboard) | `ui/theme.py`, `ui/components/drivers.py` | One accent per semantic state; red marks the adverse direction, green the favourable one | **Yes, after a correction.** `CLASS_COLOURS` (Part 5's five epistemic classes) each get their own colour — a wider palette than "one accent" strictly implies, but each colour still marks a distinct *kind of claim*, not an emphasis choice. **This row previously claimed red appeared "only" on contradicting-evidence cards and hard violations. That was false**, and the S1 hero screenshot disproves it at a glance: `drivers.py` paints the conversion-rate bar (−57,959) red. The product/judge audit caught it. Resolved in favour of the chart rather than against it — red-means-down is the stronger mental model in a revenue decomposition, and recolouring it to protect a narrower reservation would have cost more comprehension than the reservation was worth. The rule is now the broader and actually-true one: **red = adverse** (negative contribution, or evidence arguing against the hypothesis); **green = favourable**. The driver chart states this inline beneath the bars, so the rule is visible where it is used rather than only in this file. |
